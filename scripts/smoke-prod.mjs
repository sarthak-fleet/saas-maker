...
const directoryChecks = [
  ...
  {
    name: 'Machine-readable directory exposes 54 identities',
    fn: async () => {
      const res = await fetch(`${DIRECTORY}/projects.json`);
      if (res.status !== 200) throw new Error(`status ${res.status}`);
      const body = await res.json();
      if (!Array.isArray(body) || body.length !== 54) {
        throw new Error(
          `expected 54 JSON entries, got ${Array.isArray(body) ? body.length : 'non-array'}`
        );
      }
    },
  },
