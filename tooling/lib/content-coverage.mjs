...
export function inventoryRegistryProduct(productId, options = {}) {
  ...
  const project = JSON.parse(readFileSync(projectsPath, 'utf8')).projects?.find(
    (entry) => entry.id === productId,
  );
  if (!project) throw new Error(`unknown catalog product: ${productId}`);
  const metadata = existsSync(registryPath)
    ? JSON.parse(readFileSync(registryPath, 'utf8')).products?.find(
      (entry) => entry.id === productId,
    )
    : null;
  const product = {
