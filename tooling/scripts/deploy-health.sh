...
is_out_of_fleet_repo() {
  ...
  case "$repo_name" in
    open-historia|truehire|companion-robot|device-net-test|forecast-lab|saas-maker-ci-fix)
      return 0
      ;;
  esac
...
project_has_worker_sha_tag() {
  ...
  # Some projects keep Wrangler and its immutable SHA tag in committed deploy
  # scripts rather than workflows or package manifests. Include those scripts
  # so the standards check agrees with the provider parity check.
